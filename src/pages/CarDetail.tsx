...
const CarDetail = () => {
    ...
    if (car && car.car_type_id) {
      const fetchCarType = async () => {
        const { data, error } = await supabase
          .from("car_types")
          .select("name")
          .eq("id", car.car_type_id)
          .single();
        if (!error && data) {
          setCarTypeName(data.name);
        } else {
          setCarTypeName("");
        }
      };
  ...
  if (carTypeName) {
    specs.push({ icon: CarIcon, label: "Car Type", value: carTypeName });
  }
        ...
        <div className="grid lg:grid-cols-2 gap-12">
          ...
          >
            ...
            <div className="relative h-96 lg:h-[500px] rounded-lg overflow-hidden mb-4 border-4 border-attar-gray">
              <img
                src={car.images[selectedImage] || homeImage}
